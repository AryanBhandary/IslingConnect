...
import { LuCalendar, LuUser, LuMail, LuPhone, LuClock } from "react-icons/lu";
  ...
  return (
    <div className="mx-4 sm:mx-6 lg:mx-8 my-5">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6">
        <h1 className="font-bold text-lg sm:text-xl">PAT Appointments</h1>
        ...
        {STATUS_FILTERS.map((s) => (
          <button
            ...
            className={`px-5 py-2 rounded-full text-xs font-semibold transition-all duration-200 ${
              statusFilter === s
                ? "bg-[var(--pat)] text-white shadow-md"
      ...
      {loading ? (
        <div className="flex items-center justify-center p-20">
          <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[var(--pat)]" />
        </div>
      ) : filtered.length === 0 ? (
          ...
          <table className="w-full text-left text-sm min-w-[600px]">
            <thead className="bg-[var(--gray-bg)] sticky top-0">
              <tr>
                ...
                <th className="p-3 font-semibold text-gray-600">Topic</th>
                ...
                <tr key={app._id} className="border-b border-[var(--gray-border)] hover:bg-gray-50 transition-colors">
                  <td className="p-3">
                    <div className="flex items-center gap-2">
                      <LuUser size={14} className="text-[var(--pat)] shrink-0" />
                  ...
                  <td className="p-3">
                    <div className="flex flex-col gap-0.5">
                      <div className="flex items-center gap-1.5 text-xs text-gray-500">
                        <LuMail size={12} className="text-[var(--pat)]" /> {app.registeredEmail}
                      </div>
                      <div className="flex items-center gap-1.5 text-xs text-gray-500">
                        <LuPhone size={12} className="text-[var(--pat)]" /> {app.registeredPhone}
                      </div>
                    </div>
                  </td>
